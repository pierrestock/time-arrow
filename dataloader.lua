...
function DataLoader:accuracyPerVideo(opt, model)
    ...
    while counter < 36 do
        ...
        counter = counter + 1
        local probsSumForward = torch.zeros(opt.nClasses):float()
        local probsSumBackward = torch.zeros(opt.nClasses):float()
        -- sample 10 forward triplets
        local isBackward = torch.random(0, 1)
        for j = 1, 20 do
            local seed = i + torch.random(1, 50)
            if videoNames[i] == videoNames[i + 200] then
              local seed = i + torch.random(1, 150)
            end
        ...
        stats = {}
        stats.F = probsSumForward / 20
        stats.B = probsSumBackward / 20
        ...
        if videoNames[i] == videoNames[i  + 50] then
            ...
            if videoSplits[videoNames[i]] == opt.garbageClass then
                ...
                if seed % 2 == 0 then
                    ...
                    for k = 20, 29 do
                        table.insert(triplets, {i + 50, i + k + shift, i})
...
function DataLoader:run()
   ...
   local function enqueue()
      while idx <= (size - batchSize) and threads:acceptsjob() do
         local indices = perm:narrow(1, idx, math.min(batchSize, size - idx))
         local targets = perm_targets:narrow(1, idx, math.min(batchSize, size - idx))
         threads:addjob(
            function(indices, targets, nCrops)
               ...
               for i, idx in ipairs(indices:totable()) do
                  _G.dataset.hsplit = torch.random(0, 1)
                  _G.dataset.cropdId = torch.random(1, 10)
                  for j = 1, 3 do
